...
router.get('/lists', requireToken, (req, res, next) => {
  List.find({owner: req.user.id}).populate('items')
    .then(lists => {
...
router.get('/lists/:id', requireToken, (req, res, next) => {
  ...
  List.findById(req.params.id).populate('items')
    .then(handle404)
...
// CREATE
// POST /examples
router.post('/lists', requireToken, removeBlanks, (req, res, next) => {
...
router.delete('/lists/:id', requireToken, (req, res, next) => {
  ...
  List.findById(listId)
    ...
    .then(() => Item.deleteMany({list: listId}))

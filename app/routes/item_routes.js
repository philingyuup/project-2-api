...
router.get('/items', requireToken, (req, res, next) => {
  Item.find({owner: req.user.id}).populate('owner').populate('list')
    .then(items => {
...
router.post('/items/name', requireToken, (req, res, next) => {
  ...
  Item.find({name: req.body.item.name, owner: req.user.id}).populate('owner').populate('list')
    .then(handle404)
    .then(items => {
      // `examples` will be an array of Mongoose documents
      // we want to convert each one to a POJO, so we use `.map` to
...
// CREATE
// POST /examples
router.post('/items', requireToken, removeBlanks, (req, res, next) => {
